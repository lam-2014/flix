...
9) Write the `create` action in the Sessions controller
...
15) Define the `update` action in the Users controller
...
16) Right now, everyone can edit user information, so we should implement some controls
...
17) Add the `index` action and view, to show all the users present in the website
...
19) Add pagination to the `index` view and action
...
21) Let the admin delete other users
...
- move the `signed_in_user` method from the Users controller to the `SessionHelper`
...
- add three indexes on `follower_id` and `followed_id` (by acting on the migration), one for each of them 
and one for ensuring that a user cannot follow another user more than one time
